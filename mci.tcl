...
#   maximum we use a naive aproach: sample the hypercube defined by the ranges
#   of a function with a delta-hypercube of side determined by the "step"
#   option and return the smallest and largest value found. The full algorithm
#   is presented in the README file. When we've found the desired values, we
#   update the passed in "fnData" array (completing the "Min" and "Max" keys
#   of the selected function) and return it to the caller.
# Arguments:
...
proc getFunctionMinMax {fnName arClData arFnData} {
    ...
    # This holds the current position in the range hypercube. Its initial value
    # is the first point in the range hypercube.
...
#   into a proper format and extract some information from the data.
#   Integration is done by the Monte Carlo Method. The README better describes
#   this algorithm.
# Arguments:
#   fnName: the name of a function. This must be present in the "fnData" array.
#   arClData: the "array get" representation of "clData".
...
proc integrateFunction {fnName arClData arFnData} {
    ...
    # case it is truly an area, while in later cases is is a hyper-volume (in
    ...
    # number of sample points gives us a measure of how much the hyper-volume
    ...
    # "area" gives us the approximate value of the definite integral. Again,
...
#   from Tcl and produces an "array get" representation of "clData". The format
#   of "clData" is described in README, but it should be pretty obvious from
#   the code. This function assumes it's been called after "checkCommandLine"
...
#   "clData" array. The format of "fnData" is described in README, but reading
#   this function might shed some light on it as well. The function assumes
#   it's been called after "checkFnData" and whatever is in "fnText" is valid.
#   Also, it makes the assumption, further described in README, that "fnText"
#   can be interpreted as a Tcl list, although, strictly speaking, it is just
...
proc parseFnData {fnText} {
	...
	# and transform every variable reference (rule expression syntax is
	# described in README) to names which won't cause problems when using
	# Tcl's "eval" to evaluate the expression.
...
proc postCheckFnData {arClData arFnData} {
    ...
    # values used are the ones at the beginning of our range hyper-cube.
